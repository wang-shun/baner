...
public class DecryptBusinessService implements BusinessService {

	static Logger logger = Logger.getLogger(DecryptBusinessService.class);
	...
	public CommonContext service(CommonContext context) throws ServiceException {
		...
		try{
			try {
				...
				data = new String(Base64Util.decode(((String)context.getObj(ContainerConstantField.MSG_SECRET,CommonContext.SCOPE_GLOBAL)).getBytes(encoding)), encoding);
				if (logger.isDebugEnabled()) {
					logger.debug("get original message success ! message is ["+ data + "] , encoding = [" + encoding + "]");
				}
	
			} catch (UnsupportedEncodingException e) {
				logger.error("trans orginal message to String error !", e);
				// 转换信息失败则无法进行后续处理,直接返回！
				context.setErrorCode(BusinessErrorCodeConstant.BUSS_PLA000519);
				return context;
			}
			...
			if (leftMessage.length() > MESSAGE_FLAG_DIGIT) {
				...
				if (flagChar == ENCRYPTION_FLAG) {
					...
					try {
						...
						// 将获取到的RSA签名还原。
						byte[] sign = Base64Util.decode(signencode.getBytes(encoding));
						...
						if (flag) {
							...
							context.setErrorCode(BusinessErrorCodeConstant.BUSS_PLA000536);
							return context;
						}
