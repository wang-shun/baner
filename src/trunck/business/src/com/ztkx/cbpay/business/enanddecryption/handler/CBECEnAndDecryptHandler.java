...
/**
 * 中信银行加密加签，解密验签的功能实现类
 * 暂时不用，代码中注错误码的地方都被注释了
 * @author tianguangzhao
 *
 */
public class CBECEnAndDecryptHandler implements EnAndDecryptHandler {

	static Logger logger = Logger.getLogger(CBECEnAndDecryptHandler.class);
	...
	private void init(CommonContext context) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("start init CBECEnAndDecryptHandler ! ");
		...
		try {
			...
			isInit = true;
			if (logger.isDebugEnabled()) {
				logger.debug("init CBECEnAndDecryptHandler success ! ");
			}

		} catch (Exception e) {
			logger.error("init CBECEnAndDecryptHandler error ! ", e);
		}
...
//						context.setErrorCode(ErrorCodeConstant.BASE_PLA000023);
						return context;
					}

				} catch (Exception e) {
					logger.error("decrypt message error ! ", e);
//					context.setErrorCode(ErrorCodeConstant.BASE_PLA000024);
					return context;
				}
