...
public class LoadAndCheckOrderBuss implements BusinessService {
	...
	public CommonContext service(CommonContext context) throws ServiceException {
		...
		List<Map<String,String>> cyclData = (List<Map<String,String>>)context.getObj("payBatList", CommonContext.SCOPE_GLOBAL);
		...
		try{
			...
			for (int i = 0; i < orderList.size(); i++) {
				...
				// 检查订单购汇状态
				PABBusiUtil.payExgcheckBuyExgStatus(context, order);
				// 检查账户划转状态
				PABBusiUtil.payExgCheckAcTStatus(context, order);
				// 检查订单付汇状态
				PABBusiUtil.checkPayExgStatus(context, order);
			}
