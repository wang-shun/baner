...
<body style="background-color: #F0F0F0; margin-left: 170px">
	...
	<div id="p" class="easyui-panel" title="订单信息" iconCls="icon-ok"
		...
		<table style="width: 90%; margin-left: 20px; height: 200px">
			...
			<tr>
				<td class="f2">订单金额:</td>
				<td><fmt:formatNumber value="${merchantOrderMsg.body.totalAmount }" type="currency" pattern="#,#00.00"/>元</td>
				<td class="f2">订单币种:</td>
				<td>${merchantOrderMsg.body.currency }</td>
			</tr>
			<tr>
				<td class="f2">收单金额:</td>
				<td><fmt:formatNumber value="${orderShowRepBean.acceptanceMount }" type="currency" pattern="#,#00.00"/>元</td>
				<td class="f2">收单币种:</td>
			...
			<tr>
				<td colspan="4">
					<c:choose> 
						<c:when test="${checkOrderReqBean.poundageFlag=='1'}">
							支付金额=订单金额*汇率+手续费
						</c:when>
						<c:otherwise>   
								支付金额=订单金额*汇率
				...
				$.ajax({
					...
					success : function(data) {
						...
						if(result[0]=="true"){
							...
							alert("验证码错误");
						}else if(result[0]=="infofail"){
							alert("实名认证错误");
						}else if(result[0]=="payfail"){
							alert("支付错误");
						}else{
							alert("支付系统错误");
						}
