...
import cn.msec.rest.web.bean.FieldsMapperBean.SearchField;
import cn.msec.rest.web.bind.FieldUtils;
import cn.msec.rest.web.bind.QueryMapperResolver;
...
public class SqlMaker {
	...
	public static <T> String getReferenceCountSql(DbCondi dc)
		...
		try {
			...
			return selectInfoSql(" COUNT(1) COUNT ", dc); 
		}
	...
	public static String getDbFieldNames(Class<?> clazz,FieldsMapperBean fmb){
		...
		if(fields.lastIndexOf(",")>-1)
		 s = fields.substring(0, fields.lastIndexOf(","));
	...
	public static void addPageLimit(PageInfo para,StringBuffer sql){
		if(para!=null){
			...
			if(Integer.MAX_VALUE != para.getLimit()){
				...
				sql.insert(0, "select * from ( select tmp.*,rownum num from (")
				.append(") tmp ) t where t.num > ").append(para.getSkip()).append(" and t.num <= ").append(para.getSkip()+para.getLimit());
			}
		}
	}
