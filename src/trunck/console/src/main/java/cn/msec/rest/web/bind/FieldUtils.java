...
public class FieldUtils {
	...
	public static String getValue(Field field,String key){
		if(key.contains(field.getName())){
			key = key.substring(key.indexOf(field.getName()));
			return key.substring(key.indexOf(field.getName())+field.getName().length()+1,key.indexOf("_"));
		}
		return null;
	}
}
