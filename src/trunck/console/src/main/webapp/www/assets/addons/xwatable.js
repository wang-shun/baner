...
//                });
                ...
                if(xwajson.xwFlag=="prodbalance"){
                	...
                	for(var i=0;i<selrows.length;i++){
                		...
                		//查询结算日期里是否存在没有处理的差错，如果有请先处理
                		var ret1=Restful.find(restURL+"/setterror/"+sel.SETT_DATE+","+sel.PROD_ID,null);
                		console.log("差错处理"+JSON.stringify(ret1))
                		if(ret1.success==false){
                			bootbox.alert("<br><center><h4>" + ret1.description + "</h4></center><br>");
                			return;
                		}
                		ids.push(sel.ID);
                		var accttype = sel.ACCT_TYPE.split("@")[0];
                	    if(accttype =='0') kaihucou+=sel.TOTAL_CNT ;
                	    else if(accttype=='1') shengou+=sel.TOTAL_AMT,shengoucou+=sel.TOTAL_CNT;
                	    else if(accttype=='2') shuhui+=sel.TOTAL_AMT,shuhuicou+=sel.TOTAL_CNT;
                	    else if(accttype=='3') zhuanrang+=sel.TOTAL_AMT,zhuanrangcou+=sel.TOTAL_CNT;
                	    else if(accttype=='4') shourang+=sel.TOTAL_AMT,shourangcou+=sel.TOTAL_CNT;
                	    else if(accttype=='5') shouyi+=sel.TOTAL_AMT,shouyicou+=sel.TOTAL_CNT;
                	    else if(accttype=='6');
                	    else if(accttype=='7') ;
                	}
...
//                                    var ret=waTable.update();
                    ...
                    buttons: {
                        success: {
                            ...
                            callback: function() {
                                ...
                                Restful.insert(
                                		wpfadmin+"exsysoplogs/saveAuditLogInfo",{"id":Math.uuid(32),"crtTime":new Date(),"level":modjson[modjson.length-1].default_value=="01"?"00":"03","skeys":$.cookie('userinfo').username,"contents":auditTableObj.find("#auditOpini").val()}
                                );
                                waTable.update();
                            }
                        },
                        danger: {
                            ...
                            callback: function() {
                                ...
                                Restful.insert(
                                		wpfadmin+"/exsysoplogs/saveAuditLogInfo",{"id":Math.uuid(32),"crtTime":new Date(),"level":modjson[modjson.length-1].default_value=="01"?"03":"00","skeys":$.cookie('userinfo').username,"contents":auditTableObj.find("#auditOpini").val()}
                                );
                                waTable.update();
                            }
                        },
            ...
            var cardinfo=function (selrow){
                var url = wpfrisk + 'busercard/cardinfo';
                var datajson = {
                		'merchantid':selrow.merchantid,
                		'purchaserid':selrow.purchaserid
                	}
                var jsonData = Restful.cardinfo(url,datajson);
                var strvalue="";
                if(jsonData.rows.length==0){
                	bootbox.alert("<br><center><h5>暂无有效银行卡！</h5></center><br>");
                }else{
	                $.each(jsonData.rows, function() {
	                	strvalue=strvalue+"<br><center><h5>账号："+this.cardid+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;银行："+this.bankname+"</h5></center><br>"
	                })
	                bootbox.alert(strvalue);
                }
            };
...
var redoFunction=function(){
	var selrows = waTable.getData(true);
    if (selrows.rows.length == 0) {
        bootbox.alert("<br><center><h4>请选择需要结算的行</h4></center><br>");
    }else if (selrows.rows.length > 1) { 
        bootbox.alert("<br><center><h4>一次只能结算一行</h4></center><br>");
    } else {
    	tbform = Duster.buildArr($('#__dust_tableform'));
        var selrow = selrows.rows[0];
        RunForRow(selrow);
    	/*if(xwajson.xwFlag=="mchntprodbalance"){
        	updForRowss(selrows.rows)
        }*/
    	/*if (selrows[0].status!="0000"){
    	bootbox.alert("<br><center><h4>所选择行不是初始状态，不能结算</h4></center><br>");
    	}*/
    	//var selrow = selrows.rows;
    	var url = wpfadmin+"/resendRequest"
    	$.ajax({
    		type: "POST",
    		url: url,
    		data:{mchntId:selrow[0].mchntId,prodId:selrow[0].prodId,settDate:selrow[0].settDate},
    		success: function(msg){
    			if(msg!="succ"){
    				alert("结算失败:"+msg);
    			}
    		...
    		error:function(data, textStatus, xhr){
    			alert("结算失败，服务器返回状态码"+data);
    		}
    		}); 	
}
};
var RunForRow=function (selrow){
    if(xwajson.readonly==true){
        return;
    }
    //var modjson = JSON.parse(JSON.stringify(coldefs));
    if(xwajson.xwFlag=="mchntprodbalance"){
 	   if(selrow.status=="0001"){
 		   bootbox.alert("<br><center><h4>您所选择的行是已结算状态，请重新选择</h4></center><br>");
 		   return;
 	   }
                             ...
                             if(mm['col']==val.name&&mm['validate']!=undefined)
                               ...
                               if(xwaOptions['onUpdated']&&(xwaOptions['onUpdated'](selrow[xwaOptions["key_column"]],moditem))==1){
                            	   ...
                            	   if(xwajson.xwattr=="productaudit"){
                            		   ...
                            		   Restful.update(wpfadmin+"exlandshelvesmag",selrow[xwaOptions["key_column"]],
                            	   ...
                            	   Restful.insert(
                            			   wpfadmin +"exsysoplogs/saveAuditLogInfo",{"id":Math.uuid(32),"crtTime":new Date(),"level":"1","skeys":$.cookie('userinfo').username,"contents":modjson[4].default_value}
                                      );
                                   var ret=waTable.update();
                               }
                           }
                          ...
                          $.each(modjson, function(index, mm) {
                             ...
                             if(mm['col']==val.name&&mm['validate']!=undefined)
                               ...
                               if(xwaOptions['onUpdated']&&(xwaOptions['onUpdated'](selrow[xwaOptions["key_column"]],moditem))==1){
                                   ...
                                   Restful.insert(
                                		   wpfadmin+"exsysoplogs/saveAuditLogInfo",{"id":Math.uuid(32),"crtTime":new Date(),"level":"2","skeys":$.cookie('userinfo').username,"contents":modjson[4].default_value}
                                   );
                                   waTable.update();
                               }
                           }
           ...
           var exportFucntion = function(){
            ...
            xwajson['rowDblClicked']=function(e){
            	if(xwajson.xwattr=="audit"){
            	    ...
            	    dtlForRow(e.row);
            	}else if(xwajson.xwattr=="cardinfo"){
            		cardinfo(e.row);
...
//-----------------READY------
            ...
            if(xwajson['redo']) $('#'+xwajson['redo']).click(addFunction);
            else $('#redo_button').click(redoFunction);//redo-button
